...
static grpc_channel_args* get_secure_naming_subchannel_args(
  ...
  grpc_channel_args_destroy(args_with_authority);
  grpc_channel_args* final_sc_args =
      static_cast<grpc_channel_args*>(gpr_malloc(sizeof(*final_sc_args)));
  memcpy(final_sc_args, args, sizeof(*args));
  final_sc_args = new_args;
  return final_sc_args;
}

static grpc_subchannel* client_channel_factory_create_subchannel(
    grpc_client_channel_factory* cc_factory, const grpc_channel_args* args) {
  grpc_channel_args* final_args = get_secure_naming_subchannel_args(args);
  if (final_args == nullptr) {
    gpr_log(
        GPR_ERROR,
  ...
  grpc_subchannel* s = grpc_subchannel_create(connector, final_args);
  grpc_connector_unref(connector);
  grpc_channel_args_destroy(const_cast<grpc_channel_args*>(final_args));
  return s;
}
